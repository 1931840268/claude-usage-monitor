...
各字段含义：当前模型（含effort档位）、本会话/今日/当前5小时窗口成本、官方5小时限额百分比与刷新倒计时（订阅账号；API Key用户自动换成本地估算的窗口进度）、7天限额百分比、上下文窗口占用、当前燃烧率（tokens每分钟，2000以下🔥正常、2000～5000⚠️偏高、5000以上🚨很高）。
...
- `daily_budget_usd`：当日成本达到预算80%时状态栏出现黄色「⚠预算」，超过100%变红色「🚨超预算」。
